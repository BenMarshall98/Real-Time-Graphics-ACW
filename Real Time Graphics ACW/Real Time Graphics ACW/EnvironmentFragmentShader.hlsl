...
float4 main(VS_OUTPUT input) : SV_Target
{
    ...
    float3 baseColor = baseTexture.Sample(Sampler, input.PosDir).xyz;
    float spec = 32;
    ...
    if (DirectionalUsed)
    {
        ...
        color += lerp(baseColor.xyz, inkColor * 0.1f, inkFactor) * DirectionalColor.xyz * float3(0.1f, 0.1f, 0.1f);
        color += lerp(baseColor.xyz, inkColor, inkFactor) * DirectionalColor.xyz * shadow * diffuse;
        color += lerp(baseColor.xyz, inkColor, inkFactor) * DirectionalColor.xyz * shadow * specular;
    }
    ...
    if (PointUsed)
    {
        ...
        color += lerp(baseColor.xyz, inkColor * 0.1f, inkFactor) * PointColor.xyz * attenuation;
        color += lerp(baseColor.xyz, inkColor, inkFactor) * PointColor.xyz * diffuse * shadow * attenuation;
        color += lerp(baseColor.xyz, inkColor, inkFactor) * PointColor.xyz * specular * shadow * attenuation;
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        if (SpotUsed[i])
        {
            ...
            color += lerp(baseColor.xyz, inkColor * 0.1f, inkFactor) * SpotColor[i].xyz * attenuation * shadow * intensity;
            color += lerp(baseColor.xyz, inkColor, inkFactor) * SpotColor[i].xyz * diffuse * attenuation * shadow * intensity;
            color += lerp(baseColor.xyz, inkColor, inkFactor) * SpotColor[i].xyz * specular * attenuation * shadow * intensity;
        }
    }
