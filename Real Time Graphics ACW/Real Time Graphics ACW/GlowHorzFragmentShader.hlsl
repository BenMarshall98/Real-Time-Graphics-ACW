...
PS_OUTPUT main(VS_OUTPUT input)
{
    ...
    for (int i = 1; i < 5; ++i)
    {
        color += colorTexture.Sample(Sampler, input.TexCoord + float2(texOffset.x * i, 0.0)) * weight[i];
        color += colorTexture.Sample(Sampler, input.TexCoord - float2(texOffset.x * i, 0.0)) * weight[i];
        
        if (outDepth == 1.0f)
        {
            outDepth = depthTexture.Sample(Sampler, input.TexCoord + float2(texOffset.x * i, 0.0)).r;
        }
        else
        {
            depth = depthTexture.Sample(Sampler, input.TexCoord + float2(texOffset.x * i, 0.0)).r;
